...
export const ipv4AddressingQuestions: QuizQuestion[] = [
  ...
  },
  {
    ...
    options: [
      "Because they use private encryption keys that cannot be shared over the public internet.",
      "Because Internet Service Providers (ISPs) drop RFC 1918 addresses, as they are not globally unique.",
      "Because they rely on deprecated routing protocols that are not supported by modern ISPs.",
      "Because they cannot carry routing information in their header when traversing the internet.",
    ],
    correctAnswer: 1,
    difficulty: "easy",
    explanation:
  ...
  },
  {
    ...
    options: [
      "A packet sent to every host on the local physical network segment.",
      "A packet sent to all host bits set to 1 for a specific remote subnet.",
      "A packet sent to a set of hosts subscribing to a single multicast group.",
      "A packet sent to a single host using its unique destination address.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "Dividing a single large network into multiple smaller subnets to conserve address space allocation.",
      "Combining multiple contiguous smaller networks into a single larger network route to reduce routing table size.",
      "Translating private network addresses into public addresses so hosts can reach the global internet.",
      "Encrypting routing protocol updates to prevent unauthorized devices from modifying the network routing table entries.",
    ],
    correctAnswer: 1,
    difficulty: "easy",
    explanation:
  ...
  // --- IP Header & Hex/Binary Basics ---
  {
    ...
    options: [
      "To specify the quality of service priority assigned to the packet by the sending device.",
      "To prevent packets from endlessly looping in the network by decrementing at each router hop",
      "To determine the maximum time the packet can remain buffered before being discarded by the router.",
      "To store the position of the data fragment within the original packet for proper reassembly.",
    ],
    correctAnswer: 1,
    difficulty: "easy",
    explanation:
