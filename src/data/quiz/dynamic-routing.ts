...
export const dynamicRoutingQuestions: QuizQuestion[] = [
  ...
  },
  {
    ...
    options: [
      "To automatically assign IP addresses to hosts on the local network.",
      "To securely encrypt all traffic passing between neighboring routers.",
      "To dynamically discover remote networks and maintain accurate routing tables.",
      "To translate private IP addresses to public addresses for Internet access.",
    ],
    correctAnswer: 2,
    difficulty: "easy",
    explanation:
  ...
  },
  {
    ...
    options: [
      "Metric used to rank route preference",
  ...
  },
  {
    ...
    options: [
      "To completely shut down the interface and remove it from all routing protocol operations.",
      "To prevent routing updates from being sent out the interface, while still advertising the interface's network.",
      "To allow the interface to only receive routing updates from its neighbors, but never send any out.",
      "To configure the interface as a backup path for use by a floating static route when OSPF fails.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "A feature that allows a router to load balance traffic over multiple unequal cost paths at the same time.",
      "A loop-prevention mechanism that prevents a router from advertising a route back out the interface it was learned on.",
      "A method for dividing a large autonomous system into smaller, more manageable OSPF areas to scale.",
      "A technique for injecting a default route into an IGP to provide external network reachability to clients.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "Injecting false routing updates into the network to perform a Man-in-the-Middle style attack.",
      "Advertising a down network with an infinite metric to ensure all routers instantly know it is unreachable.",
      "Filtering specific routes using a route-map policy to control route redistribution between protocols.",
      "Translating an IPv4 route into an equivalent IPv6 route entry for dual-stack networks.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  // --- OSPF Basics & Router ID ---
  {
    ...
    options: [
      "Hop count to the destination",
      "Delay and bandwidth of the links",
      "Cost (based on interface bandwidth)",
      "Reliability of the link path",
    ],
    correctAnswer: 2,
    difficulty: "easy",
    explanation:
  ...
  },
  {
    ...
    options: [
      "Loopback interfaces have a much higher default bandwidth than physical interfaces, improving metrics.",
      "Loopback interfaces are logical and never go down (unless manually shut down), ensuring OSPF stability.",
      "Loopback interfaces automatically bypass all configured firewall and security filtering rules.",
      "Loopback interfaces are required for all OSPF backbone areas to function correctly in a design.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "To store the OSPF neighbor passwords and authentication keys in the database.",
      "To maintain an identical map of the network topology among all routers in an area.",
      "To filter malicious BGP routes from entering the OSPF routing domain entirely.",
      "To hold only the best routes for the forwarding routing table of the router.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "Mismatched Hello and Dead timers between the two routers",
      "Mismatched OSPF Area IDs on the connecting interfaces",
      "Mismatched MTU (Maximum Transmission Unit) size on the interfaces",
      "Duplicate Router IDs configured on the two routers",
    ],
    correctAnswer: 2,
    difficulty: "hard",
    explanation:
  ...
  },
  {
    ...
    options: [
      "They are maintained, but only incoming routing updates are filtered out.",
      "They drop, because Hello packets are no longer sent or processed on that interface.",
      "They convert to the 2-Way state and remain there until the interface is fixed.",
      "Nothing, because the passive-interface command only affects EIGRP, not OSPF.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "To act as the default gateway for all end host PCs that are connected to the segment.",
      "To collect LSAs from all routers on the segment and flood them to the rest of the segment.",
      "To translate OSPF routes into BGP route advertisements for redistribution.",
      "To assign IP addresses to newly attached routers on the multiaccess segment.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "All routers immediately hold a new DR election based on the highest Router ID.",
      "The BDR immediately promotes itself to DR, and a new election is held for the BDR position.",
      "OSPF routing stops entirely until an administrator resets the OSPF process.",
      "The DROTHER with the highest priority immediately preempts the role.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "To enable IPv6 routing so that OSPFv3 can run alongside the existing IPv4 OSPF process on the router.",
      "Because the default 100 Mbps reference bandwidth makes FastEthernet, Gigabit, and 10-Gigabit all tie with a cost of 1.",
      "To prevent unauthorized routers from joining the OSPF routing domain without the correct authentication credentials.",
      "To force OSPF to calculate its routing metric using end-to-end delay values rather than interface bandwidth.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "Select the path with the highest next-hop IP address.",
      "Select the path with the lowest next-hop IP address.",
      "Perform Equal Cost Multipath (ECMP) load balancing across both paths.",
      "Drop the route entirely to avoid creating a routing loop.",
    ],
    correctAnswer: 2,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      "E1 routes are used for IPv4 networks while E2 routes are used for IPv6 networks within the OSPF domain.",
      "E2 metric remains static throughout the OSPF domain; E1 metric adds internal OSPF cost to the external cost.",
      "E1 routes are always preferred over Intra-Area routes within the OSPF routing hierarchy.",
      "E2 routes are generated by ABRs while E1 routes are generated by ASBRs within the OSPF network.",
    ],
    correctAnswer: 1,
    difficulty: "hard",
    explanation:
  ...
  },
  {
    ...
    options: [
      "It creates a static route to the 192.168.10.0 network which overrides all dynamic routing protocol entries.",
      "It assigns an IP address in the 192.168.10.0 range to the OSPF backbone area 0 on the local router interface.",
      ...
      "It blocks all traffic originating from the 192.168.10.0 network by applying an inbound access control list on interfaces.",
    ],
    correctAnswer: 2,
    difficulty: "medium",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "It makes the default route an E1 route instead of an E2 route, changing how the metric accumulates across the OSPF domain.",
      "It prevents the default route from expiring in the Link-State Database (LSDB) by continuously refreshing the Type 5 LSA.",
      "It also sends the default route to all configured BGP peers so they can use it in their own routing tables.",
    ],
    correctAnswer: 0,
    difficulty: "hard",
    explanation:
  ...
  },
  {
    ...
    options: [
      ...
      "The MAC address of the designated router.",
  ...
  },
  {
    ...
    options: [
      "Routing traffic rapidly within a single local corporate LAN to connect end users.",
      "Routing traffic between different Autonomous Systems, such as routing the global Internet.",
      "Electing a DR/BDR router for management purposes within the OSPF broadcast network.",
      "Replacing Spanning Tree Protocol in Layer 2 networks to prevent loops entirely.",
    ],
    correctAnswer: 1,
    difficulty: "easy",
    explanation:
  ...
  },
  {
    ...
    options: [
      "It specifies the local Autonomous System number that this router belongs to.",
      "It defines a BGP neighbor and explicitly states the neighbor's Autonomous System number.",
      "It creates a static route to the BGP neighbor's IP address in the routing table.",
      "It filters all incoming routes that originate from Autonomous System 65000.",
    ],
    correctAnswer: 1,
    difficulty: "medium",
    explanation:
